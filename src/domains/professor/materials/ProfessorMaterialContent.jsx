...
export default function ProfessorMaterialContent() {
  ...
  const applyLessonsList = useCallback((list) => {
    setCourseList(list.map((l) => ({ id: l.id })))
    const meta = {}
  ...
  const courseOptions = useMemo(
    () =>
      courseList.map((c) => ({
        value: c.id,
        label: (courseNameById[c.id] ?? '').trim() || '—',
      })),
  ...
  const lessonRows = useMemo(() => {
    ...
    const meta = lessonMetaById[selectedLessonId]
    const title = (lessonTitleById[selectedLessonId] ?? '').trim() || '—'
    return [
      {
        id: selectedLessonId,
        fileName: title,
      ...
      <div className="edu-mat__card">
        ...
        {selectedCourseId ? (
          <div className="edu-mat__course-status-block" role="status">
            ...
            {selectedLessonId ? (
              <p className="edu-mat__lesson-status">
                선택된 교안: <strong>{selectedLessonTitle}</strong>
              </p>
            ) : null}
          </div>
        ) : null}
