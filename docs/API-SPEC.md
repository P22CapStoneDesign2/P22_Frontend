...
| Method | URL | 인증 | 권한 | 설명 |
|--------|-----|------|------|------|
| `GET` | `//api/admin/professors/pending` | ✅ | ADMIN | 교수 가입 신청 목록 조회 |
| `POST` | `//api/admin/professors/{userId}/approve` | ✅ | ADMIN | 교수 가입 승인 |
| `POST` | `//api/admin/professors/{userId}/reject` | ✅ | ADMIN | 교수 가입 거절 |
...
> 성공 시 `Role.PROF` + `AuthProvider.LOCAL`로 저장됩니다. 계정 상태는 **`PENDING`** 이며, **관리자 승인(§34) 후** 로그인할 수 있습니다.
...
> - 교수(PROF) 계정이 **`PENDING`**(관리자 미승인)이면 `403` — `관리자 승인 대기 중입니다.` 등
...
> 관리자가 교수(PROF) 로컬 가입 신청 목록을 조회한다. `status` 쿼리로 필터링 가능.
...
| `page` | Integer | ❌ | 페이지 번호 (기본 0) |
| `size` | Integer | ❌ | 페이지 크기 (기본 10) |
| `sort` | String | ❌ | 정렬 (예: `requestedAt,ASC`) |
    ...
    "content": [
      {
        ...
        "nickname": "kimprof",
        "status": "PENDING",
        "requestedAt": "2026-05-24T09:00:00",
        "decidedAt": null
      }
    ],
    "totalElements": 1,
...
**POST** `//api/admin/professors/{userId}/approve` | 🔒 ADMIN

> `PENDING` 상태 교수 계정을 `APPROVED`로 변경한다. 승인 후 해당 교수는 로그인할 수 있다.
...
{
  "status": 200,
  "message": "교수 가입을 승인했습니다.",
  "data": {
    "id": 12,
    "status": "APPROVED",
    "decidedAt": "2026-05-24T10:00:00"
  }
}
```
...
**POST** `//api/admin/professors/{userId}/reject` | 🔒 ADMIN
...
| `signup_status` | VARCHAR(10) | PROF(LOCAL)만: `PENDING` / `APPROVED` / `REJECTED`. USER·ADMIN·승인된 PROF는 `APPROVED` |
| `requested_at` | TIMESTAMPTZ | PROF 가입 신청 시각 (nullable) |
| `decided_at` | TIMESTAMPTZ | 관리자 결정 시각 (nullable) |
